...
% résoudre le problème (P) en passant par la relaxation
% lagrangienne de (L) en dualisant les contraintes (11).
% -----------------------------------------------------------
function subgradient_11()
    % load data
...
function [x_k, theta_pi_k] = sub_grad(epsilon, ro, pi_zero, iterLimit, DualNoChangeTOL, a, b, c)
    ...
    while( condition )
        ...
        if gamma_k == 0
            ...
            k = k + 1;
            condition = abs(L_11(x_k,pi_k, a, b, c)-theta_pi_k) / theta_pi_k > epsilon && k<iterLimit;
        end
...
function [X, FVAL] = solve_lagrangien_relaxation(pi_k, a, b, c)
    ...
    % Compose A_10 matrix for constraints (10)
